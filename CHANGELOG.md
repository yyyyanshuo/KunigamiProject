...
### Added (新增)
- **角色资料自主维护协议**:
  - 新增人设、关系、计划各自独立的 `ADD / DELETE / EDIT / REWRITE` Action Tags，并使用支持嵌套 JSON 的统一解析器。
  - 人设按首个原文匹配增删改，AI 操作不得触碰 LOCK；完整重写必须保留全部锁定区块。
  - 关系增删改仅作用于指定对象的 description；重写支持 `[target, role, score, description]` 完整位置参数和命名字段部分更新。
  - 计划通过 date 是否存在区分有时间与无时间，以原日期状态和完整内容精确定位；修改日期时原子地从旧位置移动到新位置。
  - 日期计划升级为每个日期可保存多条独立内容；旧字符串原样迁移为单条计划，不按标点拆分。
- **人设 AI 编辑保护**:
  - 角色人设支持用独占一行的 `[[LOCK]]` 与 `[[/LOCK]]` 为核心设定添加 AI 编辑保护；标签只约束 AI 编辑，用户本人始终可以自由修改或移除。
  - 记忆页面会识别锁定区块并使用独立的锁定卡片展示，同时在编辑器内实时检查标签格式。
  - 聊天模型能够读取 LOCK 标签及内容；日常对话不解释标签，未来编辑人设时必须逐字保留已有锁定区块。
  - AI 自动生成人设会为原作或官方明确资料输出 LOCK 区块，并将推测、创作补充和互动偏好留在区块外。
  - 新增一次性上线公告和使用指南。
- **一对一角色语音电话**:
  - 聊天“更多”面板新增语音通话入口；角色也可根据完整人设与当前状态输出 `[CALL_USER]` 主动来电，并触发 Web Push。
  - 角色会使用独立的通话模型自主输出 `[CALL_ACCEPT]` 或 `[CALL_REJECT]`；默认 `gemini-3.6-flash`，用户可在个人主页手动切换，不做主观质量自动换模。
  - 全屏通话页采用半双工实时链路：Scribe v2 Realtime 转写用户语音、通话模型生成回复、Eleven v3 合成角色语音；通话音频不保存。
  - 支持 `[CALL_TONE](自然语言语气描述)`，语气原样以 `[语气]\n正文` 传给 Eleven v3；括号嵌套可安全解析。
  - 通话文字和语气保存到用户独立 SQLite；普通聊天只保留 `[voice_call](call_id)` 电话卡片，点击可查看完整文字记录。
  - STT 只读取 `.env` 的平台 `ELEVENLABS_API_KEY`；电话 TTS 与既有 TTS、试听、音色克隆继续只读取用户个人加密 Key。
  - 未配置个人 ElevenLabs Key 或角色 Voice ID 时仅关闭角色 TTS；用户仍通过平台 STT 语音输入，角色以文字回复。
- **用户语音消息**:
  - 录音过程中接入 Scribe v2 Realtime，一次性 Token 由服务端签发，平台 Key 不下发至浏览器；实时连接失败时自动降级为松手后的完整录音识别。
  - 松手后使用 Scribe v2 校正最终文本，并将笑声、叹气、背景音乐等有意义的声音事件本地化后写入语音消息文字。
  - 聊天输入栏支持切换“按住说话”，松开后自动通过 ElevenLabs Scribe 转写并发送。
  - 用户录音以 `[voice_message](短文件名)(转写文字)` 保存，私聊与群聊均可播放原始录音。
  - 录音使用平台 `.env` 中的 `ELEVENLABS_API_KEY`，媒体保存至用户 COS 路径并通过鉴权接口播放。
  - 删除语音消息时同步清理媒体登记和音频对象；无 COS 配置时自动降级到用户本地目录。
- **用户私有 ElevenLabs 音色**:
  - 个人主页支持加密保存 ElevenLabs API Key，完整 Key 永不回显。
  - TTS、语音试听和即时音色克隆使用当前登录用户自己的 Key；只有用户语音 STT 使用 `.env` 平台 Key。
  - 恢复角色记忆页的音频克隆流程，成功后自动保存 Voice ID；上传音频不在 Sakura 长期存储。
  - 移除平台每日 15 次 TTS 额度，实际 TTS 额度由用户自己的 ElevenLabs 套餐决定。
  - Voice ID 继续只保存在用户自己的角色配置中，不进入角色广场。
