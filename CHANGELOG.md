...
### Added (新增)
- **朋友圈系统 (Moments) 增强**:
  - UI 优化：在朋友圈翻译结果与点赞操作之间加入间距，提升视觉层次感。
  - 逻辑重构：角色回复用户朋友圈或评论时，内容自动包含引用的朋友圈正文，增强上下文连贯性。
- **关系图谱 (Relationship Map) 编辑器优化**:
  - 新增角色选择弹窗：在编辑关系图谱时，支持点击“+ 添加角色”直接从当前通讯录中选择，无需手动输入名字。
  - 智能过滤：选择列表会自动过滤掉当前角色本身及已在图谱中的好友，防止重复和错误输入。
  - 自动防篡改：选定角色后名字设为只读，确保数据一致性。
